# Taken from: https://github.com/pkmr06/pytorch-smoothgrad/blob/master/lib/gradients.py
...
class SmoothGrad(VanillaGrad):
    ...
    def __call__(self, x, index=None):
        ...
        for i in range(self.n_samples):
            ...
            if self.cuda:
                x_plus_noise = Variable(
                    torch.from_numpy(x_plus_noise).cuda(), requires_grad=True
                )
            else:
                x_plus_noise = Variable(
                    torch.from_numpy(x_plus_noise), requires_grad=True
                )
...
class GuidedBackpropReLU(torch.autograd.Function):
    ...
    def forward(self, input):
        ...
        output = torch.addcmul(
            torch.zeros(input.size()).type_as(input), input, pos_mask
        )
