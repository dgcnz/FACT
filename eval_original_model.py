...
def config():
    ...
    parser.add_argument(
        "--seeds", default=[42, 43, 44], nargs="+", type=int, help="Random seeds"
    )
    ...
    parser.add_argument(
        "--max-epochs", default=10, type=int, help="Maximum number of epochs."
    )
...
def eval_audio(args, seed):
    ...
    for fold in args.folds:
        ...
        if args.checkpoint is not None:
            finetuner = clip_pl.AudioCLIPClassifierTrainer.load_from_checkpoint(
                args.checkpoint
            )
        else:
            finetuner = clip_pl.AudioCLIPClassifierTrainer(
                pretrained=True, n_classes=num_classes, lr=args.lr
            )
        ...
        best_model = clip_pl.AudioCLIPClassifierTrainer.load_from_checkpoint(
...
def eval_clip(args, seed, model, train_loader, test_loader):
    ...
    if args.C is None:
        ...
        def hyperparameter_sweep():
            print(
                "Performing hyperparameter sweep to find the best regularization strength lambda."
            )
            ...
            def find_peak(l2_lambda_idx_list):
                ...
                for l2_lambda_idx in l2_lambda_idx_list:
                    classifier = LogisticRegression(
                        ...
                        C=1 / l2_lambda_list[l2_lambda_idx],
                        max_iter=100,
                        verbose=1,
                        tol=1e-8,
                    )
                    ...
                    if args.dataset == "coco_stuff":
                        ...
                        metric = (
                            np.mean((val_labels_sweep == predictions).astype(float))
                            * 100.0
                        )
            ...
            l2_lambda_init_idx = [
                i
                for i, val in enumerate(l2_lambda_list)
                if val in set(np.logspace(-6, 6, num=7))
            ]
            peak_idx = find_peak(l2_lambda_init_idx)
            step_span = 2
            while step_span > 0:
                print(step_span, "wtffffff why does this run for infinity time")
                left, right = max(peak_idx - step_span, 0), min(
                    peak_idx + step_span, len(l2_lambda_list) - 1
                )
    ...
    classifier = LogisticRegression(
        random_state=args.seed, C=C, max_iter=1000, verbose=1, tol=1e-8
    )
    ...
    if args.dataset == "coco_stuff":
        ...
        metric = np.mean((test_labels == predictions).astype(float)) * 100.0
...
def eval_cifar(args, seed):
        ...
        def hyperparameter_sweep():
            ...
            def find_peak(l2_lambda_idx_list):
                ...
                for l2_lambda_idx in l2_lambda_idx_list:
                    ...
                    predictions = classifier.predict(val_features_sweep)
                    accuracy = (
                        np.mean((val_labels_sweep == predictions).astype(float)) * 100.0
                    )
    ...
    # Perform logistic regression
    classifier = LogisticRegression(
        random_state=args.seed, C=C, max_iter=1000, verbose=1
    )
    classifier.fit(train_features, train_labels)
...
def eval_ham(args, seed):
    model, _, preprocess = get_model(
        args, backbone_name="ham10000_inception", full_model=True
    )
...
def eval_cub(args, seed):
    model, _, preprocess = get_model(
        args, backbone_name="resnet18_cub", full_model=True
    )
    _, test_loader, _, _ = get_dataset(args, preprocess)
...
def eval_isic(args, seed):
    model, backbone, preprocess = get_model(
        args, backbone_name="ham10000_inception", full_model=True
    )
    train_loader, test_loader, _, _ = get_dataset(args, preprocess)

    train_features, train_labels = get_features(
        backbone, train_loader, backbone="ham10000_inception"
    )
    test_features, test_labels = get_features(
        backbone, test_loader, backbone="ham10000_inception"
    )
...
if __name__ == "__main__":
    ...
    assert (
        metrics != {}
    ), "It appears that none of the datasets you've specified are supported."
