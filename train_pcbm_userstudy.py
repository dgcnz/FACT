...
def config():
    ...
    if args.number_of_concepts_to_prune:
        args.number_of_concepts_to_prune = [
            int(numbr) for numbr in args.number_of_concepts_to_prune.split(",")
        ]
    return args
...
def main(args, concept_bank, backbone, preprocess):
    ...
    train_embs, train_projs, train_lbls, test_embs, test_projs, test_lbls = (
    ...
    run_info, weights, bias = run_linear_probe(
        args, (train_projs, train_lbls), (test_projs, test_lbls), classes
    )
    ...
    class_original_acc = run_info["cls_acc"]["test"][index_of_class] * 100.0
    model_accuracy = run_info["test_acc"]
    current_model_state = copy.deepcopy(posthoc_layer.state_dict())

    best_greedy_model = PCBMUserStudy(
        concept_bank,
        backbone_name=args.backbone_name,
        idx_to_class=idx_to_class,
        n_classes=num_classes,
        ...
        for r in args.number_of_concepts_to_prune:
            ...
            for combination in itertools.combinations(concepts, r):
                for concept in combination:
                    posthoc_layer.prune(
                        get_concept_index(concept["concept"], concept_bank),
                        index_of_class,
                    )

                total_accuracy, class_acc = posthoc_layer.test_step(
                    (test_projs, test_lbls), args.device
                )
            ...
            run_info_file = os.path.join(
                args.out_dir, f"run_info-greedy_pcbm_{model_id}.pkl"
            )
    ...
    if args.random_pruning:
        ...
        for r in args.number_of_concepts_to_prune:
            ...
            for combination in itertools.combinations(concepts, r):
                ...
                torch.save(posthoc_layer, model_path)
                run_info_file = os.path.join(
                    args.out_dir, f"run_info-random_pcbm_{model_id}.pkl"
                )
                posthoc_layer.load_state_dict(current_model_state)
    ...
    if args.prune:
        for answer in args.prune:
            ...
            for concept_to_prune in concepts:
                posthoc_layer.prune(
                    get_concept_index(concept_to_prune, concept_bank),
                    get_class_index(args.pruning_class, idx_to_class),
                )

            pruning_accuracy, class_acc = posthoc_layer.test_step(
                (test_projs, test_lbls), args.device
            )
            pruned_class_acc = class_acc[index_of_class].item()
            ...
            torch.save(posthoc_layer, model_path)
            run_info_file = os.path.join(
                args.out_dir, f"run_info-user_pcbm_{model_id}.pkl"
            )
            posthoc_layer.load_state_dict(current_model_state)
            posthoc_layer.load_state_dict(current_model_state)
...
if __name__ == "__main__":
    ...
    if greedy_pruning_results:
        ...
        csv_path = os.path.join(
            args.out_dir, f"greedy_pruning_results_{args.dataset}.csv"
        )
    ...
    if random_pruning_results:
        ...
        csv_path = os.path.join(
            args.out_dir, f"random_pruning_results_{args.dataset}.csv"
        )
        results_df.to_csv(csv_path, index=False)
    ...
    if user_pruning_results:
        ...
        csv_path = os.path.join(
            args.out_dir, f"users_pruning_results_{args.dataset}.csv"
        )
        results_df.to_csv(csv_path, index=False)
