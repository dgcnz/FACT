...
def get_dataset(args, preprocess=None, shuffle=True, single_image=False, **kwargs):
    ...
    if args.dataset.lower() == "cifar10":
        ...
        if single_image == True:
            ...
            target_occurrence = (
                4  # Change this to change which image from the class is shown
            )

            iterator = (
                (idx, (img, label_idx))
                for idx, (img, label_idx) in enumerate(trainset)
                if classes[label_idx] == target_class_name
            )
            ...
            img, label = datasets.CIFAR100(
                root=args.out_dir, train=True, download=False, transform=None
            )[index]
        ...
        assert (
            "target" in kwargs.keys()
        ), "Please specify the target index for COCO-Stuff"
        ...
        train_loader, test_loader = load_coco_data(
            train_annot, test_annot, transform=preprocess, target=target
        )
        ...
        # For printing
        print(
            f"Evaluating COCO-Stuff Binary Classification for Class '{idx_to_class[1]}'"
        )
        ...
        train_loader, test_loader = load_siim_data(
            meta_dir, batch_size=args.batch_size, seed=args.seed
        )
        ...
        classes = sorted(
            list(set(classes))
        )  # adjust so that 0:benign and 1:malignant as in the main dataset
        ...
        dataset = load_dataset(
            "anonymous347928/pcbm_metashift", name=task_name
        ...
        dataset = load_dataset(
            "anonymous347928/pcbm_survey", name=args.dataset, use_auth_token=args.token
        )
    ...
    # the following two if-statement branches are for the extensions
        ...
        train_loader, test_loader = load_esc_data(
            meta_dir,
            batch_size=args.batch_size,
        ...
        train_loader, test_loader = load_us_data(
            meta_dir, batch_size=args.batch_size, num_workers=args.num_workers
        )
    ...
    elif args.dataset.lower() == "audioset":
        from .audioset import load_aud_data
        from .constants import AS_DIR

        train_loader, test_loader = load_aud_data(
        ...
        # for the idx_to_class variable (need to read metadata table for this)
        data_path = os.path.join(AS_DIR, "class_labels_indices.csv")
        df = pd.read_csv(data_path)
        indexes = list(df["index"])
        classes = list(df["display_name"])
        idx_to_class = {indexes[i]: classes[i] for i in range(len(indexes))}
        idx_to_class = dict(sorted(idx_to_class.items()))
        classes = list(idx_to_class.values())
