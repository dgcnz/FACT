...
def config():
    ...
    parser.add_argument(
        ...
        default=1,
        type=int,
        help="How many times to recurse on the conceptnet graph",
    )
    parser.add_argument(
        "--class_names", type=str, default="airplane,bed,car,cow,keyboard"
    )
...
def get_single_concept_data(cls_name):
    ...
    # Has relations
    has_query = (
        "https://api.conceptnet.io/query?node=/c/en/{}&rel=/r/HasA&start=/c/en/{}"
    )
    ...
    # Made of relations
    madeof_query = (
        "https://api.conceptnet.io/query?node=/c/en/{}&rel=/r/MadeOf&start=/c/en/{}"
    )
    ...
    # Parts of things
    parts_query = (
        "https://api.conceptnet.io/query?node=/c/en/{}&rel=/r/PartOf&end=/c/en/{}"
    )
...
def learn_conceptbank(args, concept_list, scenario, model=None):
    ...
    concept_dict_path = os.path.join(
        args.out_dir, f"mmc_{args.backbone_name}_{scenario}_recurse_{args.recurse}.pkl"
    )
...
if __name__ == "__main__":
    ...
    # ESC50 has 50 labels in total
        ...
        raise ValueError(
            f"Unknown classes: '{args.classes}'. Define your dataset here!"
        )
