...
def config():
    ...
    parser.add_argument(
        ...
        default=0.3,
        type=float,
        help="threshold for aggregate projections",
    )
...
def run_linear_probe(args, train_data, test_data):
    ...
    if args.dataset == "coco_stuff":
        run_info["test_auc"] = average_precision_score(
            test_labels, classifier.decision_function(test_features)
        )
        run_info["train_auc"] = average_precision_score(
...
def main(args, concept_bank, backbone, preprocess, **kwargs):
    ...
    train_projs_aggregate = compute_aggregate_projections(
        args, backbone, posthoc_layer, train_loader, test_loader
    )
    ...
    run_info, weights, bias = run_linear_probe(
        args, (train_projs, train_lbls), (test_projs, test_lbls)
    )
    ...
    model_id = (
        f"{model_id}target{kwargs['target']}"
        if (args.dataset == "coco_stuff")
        else model_id
    )
